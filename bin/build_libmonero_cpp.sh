...
if [[ $CUR_OS == "MINGW64_NT"* || $CUR_OS == "MSYS"* ]]; then
    ...
    cmake -jHOST_NCORES -DARCH=$ARCH ../.. &&
    cmake -jHOST_NCORES --build . &&
    make -j$HOST_NCORES .
    ...
    if [ BUILD_BOTH_ARCHS == 1 ]; then
        ...
        cd ../../..
        ./external/monero-project/contrib/depends/x86_64-apple-darwin11/native/bin/x86_64-apple-darwin11-lipo -create -output build/libmonero-cpp-apple-darwin.a build/x86_64-apple-darwin/release/libmonero-cpp.a build/aarch64-apple-darwin/release/libmonero-cpp.a
        ...
        # Build monero-cpp
        printf "\nBuilding ${CPU} monero-cpp for ${VERSION}\n"
        rm -rf ../../build/$VERSION/release &&
        mkdir -p ../../build/$VERSION/release && cd ../../build/$VERSION/release &&
